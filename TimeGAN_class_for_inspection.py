"""
    TimeGAN class implemented as part of the ydata_synthetic library: https://github.com/ydataai/ydata-synthetic/tree/dev
    ...
    if net_type=='GRU':
        ...
        for i in range(n_layers):
            model.add(LSTM(units=hidden_units,
                      return_sequences=True,
                      name=f'LSTM_{i + 1}')) # this was never used in our experiments

    model.add(Dense(units=output_units,
                    activation='sigmoid',     # in order to get the model to work with log returns (-1 to 1 data range) - modify this by adding a condition that if model.name == Discriminator : activation = 'sigmoid' else activation = 'tanh'
                    name='OUT'))
    return model
